import { memo, useEffect, useRef, useState } from "react";
import { useNavigate, useSearchParams } from "react-router-dom";
import AuthStatusIcon from "@/components/auth/AuthStatusIcon";
import AuthStepList from "@/components/auth/AuthStepList";
import AuthBackground from "@/components/auth/AuthBackground";
import AuthService from "@/services/AuthService";
import { useAuth } from "@/hooks/useAuth";

type AuthStatus = "loading" | "success" | "error";

const STEPS = [
    { id: 1, label: "Create a secure login session", duration: 0 },
    { id: 2, label: "Complete & redirect", duration: 300 },
];

const AuthenticatePage = () => {
    const [searchParams] = useSearchParams();
    const code = searchParams.get("code");
    const navigate = useNavigate();
    const [status, setStatus] = useState<AuthStatus>("loading");
    const [completedSteps, setCompletedSteps] = useState<number[]>([]);
    const stepIndexRef = useRef(0);
    const statusRef = useRef(status);
    const { login } = useAuth();

    useEffect(() => {
        statusRef.current = status;
    }, [status]);

    const runStep = () => {
        if (stepIndexRef.current >= STEPS.length) {
            setTimeout(() => {
                if (statusRef.current === "success") {
                    navigate("/messages", { replace: true });
                } else {
                    navigate("/");
                }
            }, 700);

            return;
        }

        const currentStep = STEPS[stepIndexRef.current];

        setTimeout(() => {
            stepIndexRef.current += 1;

            setCompletedSteps((prev) => [
                ...prev,
                stepIndexRef.current
            ]);

            runStep();
        }, currentStep.duration);
    }

    useEffect(() => {
        if (!code) {
            navigate("/");
        }

        const fetchToken = async () => {
            try {
                const response = await AuthService.loginOAuth({ code });

                if (response.code === 0 && response?.result?.token) {
                    login(response?.result?.token);
                    setStatus("success");
                } else {
                    setStatus("error");
                }
            } catch (error) {
                setStatus("error");
            } finally {
                runStep();
            }
        }

        fetchToken();
    }, [code]);

    return (
        <div className="relative min-h-screen flex items-center justify-center overflow-hidden bg-background">
            <AuthBackground />

            <div className="relative z-10 w-full max-w-md mx-auto px-6 py-10 flex flex-col items-center gap-8">
                <div className="flex flex-col items-center gap-3">
                    <div className="relative">
                        <div className="w-16 h-16 rounded-2xl bg-primary flex items-center justify-center shadow-xl shadow-primary/30">
                            <svg className="w-9 h-9 text-primary-foreground" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                <path strokeLinecap="round" strokeLinejoin="round" d="M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                            </svg>
                        </div>
                        <div className="absolute -bottom-1 -right-1 w-5 h-5 rounded-full bg-card border-2 border-background flex items-center justify-center">
                            <svg className="w-3 h-3" viewBox="0 0 24 24">
                                <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                                <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                                <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
                                <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
                            </svg>
                        </div>
                    </div>

                    <div className="text-center">
                        <h1 className="text-xl font-bold text-foreground">Verifying</h1>
                        <p className="text-sm text-muted-foreground mt-1">Please wait a moment...</p>
                    </div>
                </div>

                <div className="w-full bg-card border border-border rounded-2xl shadow-lg shadow-black/5 p-6 space-y-6">
                    <AuthStatusIcon status={status} />
                    <AuthStepList steps={STEPS} completedSteps={completedSteps} status={status} />
                </div>
            </div>
        </div>
    );
};

export default memo(AuthenticatePage);